...
class Seq2Table(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        decoder_table_outputs = decoder_table_outputs.last_hidden_state
            ...
            for j, (arg_slots, roles) in enumerate(zip(list_arg_slots_, list_roles_)):
                ...
                for (slots, arg_role) in zip(arg_slots, roles):
                    ...
                    for slot in slots:
                        ...
                        start_query = (query_sub*self.w_prompt_start).unsqueeze(-1)
                        end_query = (query_sub*self.w_prompt_end).unsqueeze(-1)
