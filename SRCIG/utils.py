...
def transfor2(label, predt):
    ...
    for i in range(len(predt)):
        # 实际标签：是否存在因果
        if label[i] >= 1:
            all_label_binary.append(1)
        else:
            all_label_binary.append(0)

        # 预测标签：是否存在因果
        if predt[i] >= 1:
            all_predt_binary.append(1)
        else:
            all_predt_binary.append(0)

    return all_label_binary, all_predt_binary
